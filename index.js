...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    const foundElem = arr.find( elem => elem > num);
    return foundElem !== undefined ? foundElem : 'Числа яке задовільняе умову не знайдено';
  } else {
    return undefined;
  }
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr)) {
    const lastElem = arr.findLast((element) => element % 2 === 0);
    return lastElem !== undefined ? lastElem : 'Парних чисел не знайдено';
  } else {
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    arr.slice(-n).forEach( elem => console.log(elem))
    return arr.slice(-n).reduceRight((accumulator, currentValue) => accumulator + currentValue);
  } else {
    return 'Вхідний параметр має бути масивом';
  }
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr)) {
    return arr.reduce(function(totalPrice, product) {
      // Додаємо ціну товару до загальної суми
      return totalPrice + product.price;
    }, 0); // Початкове значення для загальної ціни - 0
  } else {
    return 0; // Якщо вхідний параметр не є масивом, повертаємо 0
  }
}
...
function findLastNegativeNumberIndex(arr) {
  if (Array.isArray(arr)) {
    return arr.findLastIndex(function(number) {
      return number < 0;
    });
  } else {
    return -1;
